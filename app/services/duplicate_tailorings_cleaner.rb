...
class DuplicateTailoringsCleaner
  ...
  def run!
    connection.transaction do
      create_target_table
      counts = counts_from_temp_table

      if counts.fetch('tailorings').zero?
        @logger.info('DuplicateTailoringsCleaner: nothing to clean up.')
        return counts
      end
      ...
      delete_tailorings
      @logger.info("DuplicateTailoringsCleaner: cleanup committed successfully. #{summary(counts)}")
  ...
  def counts_from_temp_table
    sql = <<~SQL
      WITH target_test_results AS (
        SELECT id FROM historical_test_results WHERE tailoring_id IN (SELECT id FROM tailorings_to_delete)
      )
      SELECT
        (SELECT count(*) FROM tailorings WHERE id IN (SELECT id FROM tailorings_to_delete) AND os_minor_version IS NULL) AS null_tailorings,
        (SELECT count(*) FROM tailorings_to_delete) AS tailorings,
        (SELECT count(*) FROM tailoring_rules WHERE tailoring_id IN (SELECT id FROM tailorings_to_delete)) AS tailoring_rules,
        (SELECT count(*) FROM target_test_results) AS test_results,
        (SELECT count(*) FROM rule_results WHERE test_result_id IN (SELECT id FROM target_test_results)) AS rule_results
    SQL
    row = connection.select_one(sql)
    COUNT_KEYS.to_h { |key| [key, row.fetch(key).to_i] }
  end
  ...
  def summary(counts)
    ...
    "Removed #{counts.fetch('tailorings')} tailorings " \
