...
RSpec.describe ComplianceConsumer do
  ...
  context 'calling the appropriate service' do
    before do
    ...
    describe 'received delete message' do
      let(:service_class) { HostRemover }
      let(:type) { 'delete' }

      it 'calls the HostRemover service' do
    ...
    describe 'received create message' do
      let(:service_class) { HostAssigner }
      let(:type) { 'created' }

      it 'calls the HostAssigner service' do # TODO: naming?
        allow(@service).to receive(:assign_host)

        expect(@service).to receive(:assign_host)
